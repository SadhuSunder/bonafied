...
def generate_certificate(name, roll_number, year, semester, date, branch, fathers_name, academic_year):
    ...
    approval_paragraph = Paragraph("Approved by AICTE & PCI, Affiliated to JNTUH & SBTET, Permitted by Govt. of T.S<br/>Deshmukhi Village, Near Ramoji Film City, Behind Mount Opera, Hyderabad - 508284. T.S. INDIA", approval_style)
...
def index():
    if request.method == "POST":
        ...
        generate_certificate(name, roll_number, year, semester, date, branch, fathers_name, academic_year)
        return send_file("bonafide_certificate.pdf", as_attachment=True)
